...
def save_val_embeddings(sess, model, minibatch_iter, size, out_dir, mod=None):
    val_embeddings = []
    finished = False
    seen = set([])
    nodes = []
...
def train(train_data, num_nodes=None, test_data=None):
    G = train_data[0]
    ...
    if FLAGS.save_embeddings:
        sess.run(val_adj_info.op)
        save_val_embeddings(sess, model, minibatch, FLAGS.validate_batch_size, log_dir(), num_nodes)
...
def dynamic_test(init_file, dynamic_file, feats, id_map, flag_file):
    [G, walks, class_map, node_flag, flag_no] = init_G(init_file, flag_file, id_map, feats)
    train([G, feats, id_map, walks, class_map], G.number_of_nodes())
    change_G_status(G)
