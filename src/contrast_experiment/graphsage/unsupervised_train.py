...
def log_dir(output_path):
    log_dir = output_path + "/unsup-" + FLAGS.train_prefix.split("/")[-1]
...
def train(train_data, params, num_nodes=None, metric=None, test_data=None):
    ...
    adj_info_ori = adj_info
    if not num_nodes is None:
        adj_info = adj_info[:num_nodes+1]
    ...
    summary_writer = tf.summary.FileWriter(log_dir(params['output_path']), sess.graph)
    ...
    if not num_nodes is None:
        val_adj_info = tf.assign(adj_info, minibatch.test_adj[:num_nodes+1])
    else:
    ...
    if FLAGS.isTrain:
        ...
        # save model
        saver.save(sess, log_dir(params['output_path'])+'model.ckpt')
        print("save model succeed!")
    else:
        print("==Testing Model==")
        ckpt = tf.train.get_checkpoint_state(log_dir(params['output_path']))
...
def sage_main(params, metric=None):
    if params['isTrain']:
        ...
        f._parse_flags()
        print("==Training Model==")
        FLAGS.isTrain = params['isTrain']
        FLAGS.train_prefix = params['train_prefix']
        FLAGS.model = params['model']
        FLAGS.save_embeddings = params['save_embeddings']
        FLAGS.batch_size = params['batch_size']
        ...
        #tf.app.run(main=train_main, argv=params)

    else:
        print("==Testing Model==")
        FLAGS.isTrain = params['isTrain']
        FLAGS.train_prefix = params['train_prefix']
        FLAGS.testfile_prefix = params['test_prefix']
        FLAGS.model = params['model']
        FLAGS.save_embeddings = params['save_embeddings']
        FLAGS.batch_size = params['batch_size']
        FLAGS.validation_batch_size = params['validation_batch_size']
